...
class CI_Sqldump
{
     ...
     * @return void
     */
    function sql_dump($db, $max_size=0)
    {
        $this->db = $db;
        if ($max_size > 0 )
        {
     ...
     * @return void
     */
    function __construct($params = array('db' => null, 'max_size' => 0 ))
    {
        $this->sql_dump($params['db'], $params['max_size']);
    }
    ...
    function dump_table($path, $vol)
    {
        ...
        $this->dump_sql .= '-- END momo-crm SQL Dump Program ';
    ...
    function sql_import($sql_file)
    {
        ...
        //$sql_str = array_filter($sql_file, 'remove_comment');
        $sql_str = $sql_file;
        //$sql_str = str_replace("\r", '', implode('', $sql_str));

        if(empty($sql_str)){
            $this->error_msg = 'sql备份为空';
            return false;
        }
